...
The client opens archived content **in the page**: the archive private key never
reaches the server. Conversations, names, archived WhatsApp profile pictures,
ordinary encrypted attachments and edit history render from ciphertext. This
does not hide content from the live WhatsApp process while it is being ingested
or sent; see the [media boundaries and current limitation](docs/media-security.md).
...
reader had on screen, and ordinary encrypted attachments are stored as the
ciphertext the CDN served, without decrypting them in the download worker.
...
make mcp-http-check  # remote HTTP MCP reader: protocol, OAuth, and no content outside the enclave
make cover
make fuzz
```
...
- **The hosted metadata connector** (`server.mjs`: Wappie's
  `https://api.wappie.thehappie.co/mcp`, and the container self-hosters run
  beside their installation) holds no archive private key and cannot open
  sealed content. Names, bodies and filenames stay `locked`.
- **The attested reader** (`enclave/`: `https://mcp.wappie.thehappie.co/mcp`)
  runs the same code in an AWS Nitro Enclave whose image is published and
  verified by the browser before consent. A connection of kind `metadata`
  reads what the hosted connector reads. A connection of kind `content`, which
...
Ordinary WhatsApp attachments arrive from the Meta CDN already encrypted —
AES-256-CBC with an encrypt-then-MAC HMAC, keyed by a 32-byte `mediaKey`. The blob
is stored exactly as received; the media key, thumbnail and file name are sealed
under the device's archive key. This path currently does not require a media
key or encrypted-file hash, so it does not establish that every accepted object
is encrypted. See the [known limitation](docs/media-security.md#known-gap).
...
The server has the archive public key, but not its private half. It also holds
WhatsApp session secrets and current symmetric content keys in memory while
sealing new data; it is not isolated from live plaintext. Each device has an
archive keypair, generated by whichever client paired it; the private half is
...
- **Outbound text and media pass through server memory as plaintext.** HTTPS
  protects the browser-to-server connection. The server's WhatsApp client then
  encrypts the attachment into a temporary ciphertext file and uploads it to
  WhatsApp. Browser video conversion does not remove this boundary.

- **The attachment downloader preserves the bytes it receives.** For ordinary
  encrypted WhatsApp attachments, those bytes are ciphertext and are not
  decrypted here. When supplied, `fileEncSHA256` verifies their integrity. The
  current missing-key/hash path prevents a universal encryption-at-rest claim;
  see [media security](docs/media-security.md).
...
- **Attachment metadata is readable**: type, size, dimensions, duration, and the
  ...
  the storage boundary and limitation described above.
...
For ordinary encrypted WhatsApp attachments, the CDN supplies AES-256-CBC
ciphertext under a 32-byte media key. That ciphertext is stored verbatim, with
its trailing MAC, and the key is sealed into the message row. Object storage
does not add another application encryption layer.

`internal/media` fetches with its own HTTP client and does not decrypt the
download. When present, `fileEncSHA256` is checked without opening the content.
Missing media keys and encrypted-file hashes are currently accepted, however,
and unhashed objects share a fallback object key. Do not interpret this path as
a guarantee that every stored object is encrypted. The [security follow-up](docs/media-security.md)
describes the gap and the requirements for fixing it; it is a source review,
not evidence that an existing production object contains plaintext.
...
`-raw` retrieves the stored bytes; it is not a server storage-mode setting.
Without it, the CLI opens a supported encrypted attachment locally and writes
the decrypted file.
...
For hashed attachments, storage is content-addressed by the ciphertext hash and
prefixed by tenant, so one image forwarded into twenty chats is one object —
and two tenants holding the same file still hold it separately.
...
The current upload calls whatsmeow's `UploadReader` with the plaintext request
body received over HTTPS. It encrypts the stream into a temporary ciphertext
file before uploading to WhatsApp. The plaintext passes through server memory;
the archive subsequently fetches the encrypted CDN copy and seals the media
key. A future client-encrypted upload would need a separately reviewed protocol
and interoperability tests; it is not the behavior of this implementation.
...
WhatsApp profile pictures arrive as ordinary image bytes over HTTPS, without
the attachment media-key encryption. The server sees those bytes in memory
and seals them before storing the archived contact or group picture. Fetching
is paced, because each question is a query over the socket that carries messages.

Wappie account and workspace avatars are different: they are shared profile
metadata, stored as validated image data URLs, not encrypted archive objects.
