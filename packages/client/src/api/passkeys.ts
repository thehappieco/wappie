...
import { AuthError, authRequest, finishSession, signOut, type MeReply, type SessionReply, type SignedIn, type SignInStep } from './auth.js'
import { derive, unwrapPrivateKey, type KDFParams } from '../crypto/account.js'
import { fromBase64, toBase64, type Bytes } from '../crypto/bytes.js'
import { unwrapPasskey, wrapPasskey } from '../crypto/passkey.js'
import { endpoint } from './endpoint.js'
import { base64url, credentialJSON, creationOptions, prfOutput, publicCredential, requestOptions,
  supportsPasskeys, type CreationJSON, type RequestJSON } from './webauthn.js'
...
export async function removePasskey(input: { serverURL: string; token: string; email: string; password: string; id: string }): Promise<void> {
  ...
  const response = await fetch(endpoint(input.serverURL, '/v1/auth/passkeys/' + encodeURIComponent(input.id)), {
    ...
    body: JSON.stringify({ auth_key: derived.authKey }), cache: 'no-store', redirect: 'error',
  })
  if (!response.ok) {
