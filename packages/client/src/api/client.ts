...
export class Connection {
   ...
   */
  async request<T>(type: string, payload: unknown, wantType: string, options: {timeoutMs?: number} = {}): Promise<T> {
    const timeout = options.timeoutMs !== undefined && Number.isFinite(options.timeoutMs) && options.timeoutMs > 0 ? options.timeoutMs : REQUEST_TIMEOUT
