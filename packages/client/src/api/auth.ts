...
import { fromBase64, parseUUID, toBase64, type Bytes } from '../crypto/bytes.js'
import { importArchiveKey, type PrivateKey } from '../crypto/hpke.js'
import { grantRow, Kind, openDirect } from '../crypto/seal.js'
import { endpoint } from './endpoint.js'
import type { BrowserLogin } from '../session.js'
import { sealBrowserAccountKey, type BrowserKeyEnvelope } from '../crypto/browserAccount.js'
...
export interface Readable {
  /** Immutable cryptographic namespace; distinct from the workspace after a move. */
 ...
 * `use` also receives the immutable archive namespace. Use it, rather than the
...
async function finishWithKey(serverURL: string, reply: SessionReply, account: PrivateKey, supplied?: MeReply): Promise<SignedIn> {
  ...
  for (const g of me.grants ?? []) {
    try {
      ...
      const archiveTenantID = g.archive_tenant_id || me.user.tenant_id
      const archiveTenant = parseUUID(archiveTenantID)
      const row = await grantRow(archiveTenant, device, user, g.epoch)
      const raw = await openDirect(account, Kind.DeviceGrant, archiveTenant, row, fromBase64(g.sealed_dsk))
      try {
        readable.push({ archiveTenantID, deviceID: g.device_id, label: g.label || g.device_id.slice(0, 8), epoch: g.epoch, archive: await importArchiveKey(raw) })
      } finally { raw.fill(0) }
    } catch {
