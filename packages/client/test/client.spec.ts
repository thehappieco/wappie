...
import { setMessageResolver } from '../src/messages'
const locale = { value: 'pt' }
...
describe('protocol failure details', () => {
  it('keeps a specific server reason instead of reducing internal/conflict to a generic error', () => {
    ...
    } finally { locale.value = original; setMessageResolver() }
  })
  it('translates a known exact reason while keeping its original diagnostic and code', () => {
    const original = locale.value
    try {
      locale.value = 'en'
      setMessageResolver(() => 'This number was not found in this workspace.')
...
it('keeps a long administration request alive beyond the ordinary timeout', async () => {
 const server=await serve(() => {})
 const conn=await Connection.connect({serverURL:server.url,credential:{kind:'api_key',token:'k'}})
 vi.useFakeTimers({toFake:['setTimeout','clearTimeout']})
 let ended=false
 const response=conn.request('device.transfer',{device_id:'d'},'device.transferred',{timeoutMs:120_000})
 const observed=response.then(value=>{ended=true;return value},error=>{ended=true;return error})
