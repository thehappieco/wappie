...
async function wrappingKey(prf: Bytes, binding: PasskeyBinding): Promise<CryptoKey> {
  if (prf.length !== 32) throw new Error('A passkey não forneceu uma chave de desbloqueio válida.')
...
export async function wrapPasskey(privateKey: Bytes, prf: Bytes, binding: PasskeyBinding): Promise<Bytes> {
  if (privateKey.length !== 32) throw new Error('Chave da conta inválida.')
...
export async function unwrapPasskey(envelope: Bytes, prf: Bytes, binding: PasskeyBinding): Promise<Bytes> {
  if (envelope.length !== 61 || envelope[0] !== 1) throw new Error('O registro desta passkey está inválido. Entre com sua senha.')
  try {
    ...
    throw new Error('Esta passkey não conseguiu abrir seus dados. Entre com sua senha e cadastre uma passkey compatível.')
  }
}
