...
function describe(c: GroupChange): string {
  ...
  switch (c.action) {
    ...
    case 'promote':
      return who ? `${who} tornou ${whom} admin` : t('{v0} virou admin', { v0: whom })
    case 'demote':
...
<template>
  ...
  >
    <template #actions>
      <button class="icon-btn" @click="loadGroup(chatKey)" :title="t('Atualizar')" :disabled="busy">
        ↻
      </button>
    </template>
    ...
    <template v-else>
      <section class="section">
        <h3>{{ t('Participantes ({v0})', { v0: group.members.length }) }}</h3>
