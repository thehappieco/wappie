...
describe('passkey sign-in cleanup', () => {
  it('opens the encrypted key locally, finishes the session, and clears temporary secrets', async () => {
    ...
    const signedIn = await signInWithPasskey({ serverURL: SERVER, onProgress: progress })
    expect(progress.mock.calls.map(([step]) => step)).toEqual(['checking', 'passkey', 'authenticating', 'opening'])
